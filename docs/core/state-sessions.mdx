...
**Typed dataclass** serializable to/from JSON. Multiple session backends: `SQLiteSession`, `RedisSession`, `DaprSession`, `OpenAIConversationsSession` (server-managed threads). Interruption points frozen in `RunState` for deterministic resume.
