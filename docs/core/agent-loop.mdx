...
The Agent Loop touches every subsystem. Dive deeper into each:

- **[Startup & Bootstrap](./startup-bootstrap.mdx)** — how the CLI starts up before the loop begins
- **[Input & Rendering](./input-rendering.mdx)** — how keystrokes enter and responses render
- **[Prompt, Memory & Context](./prompt-memory-context.mdx)** — system prompt assembly, CLAUDE.md, memory, and compaction
- **[API Client](./api-client.mdx)** — streaming, retries, and multi-provider support
- **[Tool Execution](/tools/tool-execution)** — how tools run in parallel with permission checks
- **[Permission Model](/security/permission-model)** — how the permission check at step 5 works
